...
- (void)setupOnline
{
    ...
    _originalVideoFrame = CGRectMake(20, 100, 150, 150);
    self.videoViewOnline = [[GSVideoView alloc]initWithFrame:_originalVideoFrame];
    self.videoViewOnline.videoViewContentMode = GSVideoViewContentModeRatioFill;
    [self.view addSubview:self.videoViewOnline];
}

-(void)handleRoll{
    [self.broadcastManager checkinCountDownFrom:10];
}
...
-(void)btnsShow{
    ...
    [self.view bringSubviewToFront:_btnReceive];
...
-(void)btnsHide{
    ...
    [self.view sendSubviewToBack:_btnReceive];
